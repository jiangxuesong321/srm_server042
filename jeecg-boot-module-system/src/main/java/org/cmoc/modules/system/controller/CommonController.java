...
//            FileCopyUtils.copy(mf.getBytes(), savefile);
            ...
            OutputStream fileSave = new FileOutputStream(savePath);
            ...
            BufferedWriter out = new BufferedWriter(new OutputStreamWriter(fileSave,"UTF-8"));
            out.close();
...
//	}
    ...
    public static void main(String[] args) {
        ...
        int size = (str.length() % 8 == 0) ? (str.length() / 8)
                : (str.length() / 8 + 1);// 每三位取一长度
        if (str.length() < 8) { // 判断字符串的长度是否大于3
            str2 = str;
        } else {
            /*
            ...
            for (int i = 0; i < size - 1; i++) {// 前n-1段
                ...
                str2 += str.substring(i * 8, i * 8 + 8) + ",";
            }
            for (int i = size - 1; i < size; i++) {// 第n段
                str2 += str.substring(i * 8, str.length());
            }
        }
