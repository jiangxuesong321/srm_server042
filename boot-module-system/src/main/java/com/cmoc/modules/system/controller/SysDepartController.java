...
import com.cmoc.modules.system.service.ISysUserDepartService;
...
//			}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/fetchSubject", method = RequestMethod.GET)
	public Result<List<SysDepartTreeModel>> fetchSubject() {
		Result<List<SysDepartTreeModel>> result = new Result<>();
		try {
			List<SysDepartTreeModel> list = sysDepartService.fetchSubject();
			result.setResult(list);
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(),e);
