package org.cmoc.common.system.query;
...
import org.cmoc.common.constant.SymbolConstant;
import org.cmoc.common.system.util.JeecgDataAutorUtils;
import org.cmoc.common.system.util.JwtUtil;
import org.cmoc.common.system.vo.SysPermissionDataRuleModel;
import org.cmoc.common.util.CommonUtils;
import org.cmoc.common.util.DateUtils;
...
public class QueryGenerator {
	...
	/**多字段排序 TODO 需要修改前端*/
	public static String doOrder(Map<String, String[]> parameterMap) {
		String column=null,order=null;
		if(parameterMap!=null&& parameterMap.containsKey(ORDER_COLUMN)) {
			column = parameterMap.get(ORDER_COLUMN)[0];
		}
		...
		if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
			...
			// 修改为 _ 前端传递排序条件{....,column: 'column1,column2',order: 'desc'} 翻译成sql "column1 desc,column2 desc"
			//queryWrapper.orderByAsc(oConvertUtils.camelToUnderline(column));
			columnStr = oConvertUtils.camelToUnderline(column);
