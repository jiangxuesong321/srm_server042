...
public class DictAspect {
     ...
     * 定义切点Pointcut
     */
    @Pointcut("execution(public * com.cmoc.modules..*.*Controller.*(..)) || @annotation(com.cmoc.common.aspect.annotation.AutoDict)")
    ...
    private Object parseDictText(Object result) {
        if (result instanceof Result) {
            if (((Result) result).getResult() instanceof IPage) {
                ...
                Map<String, List<String>> dataListMap = new HashMap<>(5);
                //取出结果集
                List<Object> records=((List) ((Result) result).getResult());
                //update-begin--Author:zyf -- Date:20220606 ----for：【VUEN-1230】 判断是否含有字典注解,没有注解返回-----
                Boolean hasDict= checkHasDict(records);
                if(!hasDict){
                    return result;
...
//                ((List) ((Result) result).setResult(items));
            }
            ...
            Map<String, List<String>> dataListMap = new HashMap<>(5);
            //取出结果集
            List<Object> records = (List) result;
            //update-begin--Author:zyf -- Date:20220606 ----for：【VUEN-1230】 判断是否含有字典注解,没有注解返回-----
            Boolean hasDict= checkHasDict(records);
            if(!hasDict){
                return result;
